...
const LoginPage: React.FC = () => {
  ...
  if (isLoading && !currentUser) { // Show loading only if initial auth check is in progress
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-background-light to-blue-100 dark:from-background-dark dark:to-blue-900 p-4">
      <div className="w-full max-w-md p-8 bg-card-light dark:bg-card-dark rounded-xl shadow-2xl space-y-8">
        <div className="text-center">
          <SparklesIcon size={48} className="mx-auto text-primary-DEFAULT mb-4" />
          <h1 className="text-3xl font-bold text-text-light dark:text-text-dark">Welcome to {APP_NAME}</h1>
          <p className="mt-2 text-text-muted-light dark:text-text-muted-dark">Sign in to access your dashboard and models.</p>
        </div>
