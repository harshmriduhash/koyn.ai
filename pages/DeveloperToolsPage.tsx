...
const DeveloperToolsPage: React.FC = () => {
  ...
  const [language, setLanguage] = useState<"javascript" | "python" | "curl">(
    LANGUAGES_FOR_BOILERPLATE[0].value as "javascript" | "python" | "curl"
  );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setGeneratedCode("");
    try {
      const code = await generateBoilerplateCode(
        modelName,
        taskDescription,
        language
      );
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="text-center mb-12">
        ...
        <p className="text-lg text-text-muted-light dark:text-text-muted-dark">
          Integrate AI models seamlessly into your applications.
        </p>
      </div>
      ...
      <section className="mb-16 p-8 bg-card-light dark:bg-card-dark rounded-xl shadow-xl">
        <div className="flex items-center mb-6">
          ...
          <h2 className="text-3xl font-semibold text-text-light dark:text-text-dark">
            AI-Powered Boilerplate Generator
          </h2>
        </div>
        <p className="text-text-muted-light dark:text-text-muted-dark mb-6">
          Need to integrate a model quickly? Describe your task, specify the
          model, and choose your language. Our AI will generate starter code for
          you. Powered by Gemini.
        </p>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <select
              ...
              value={language}
              onChange={(e) =>
                setLanguage(e.target.value as "javascript" | "python" | "curl")
              }
              className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-text-light dark:text-text-dark focus:ring-primary-DEFAULT focus:border-primary-DEFAULT"
            >
          ...
          >
            {isLoading ? (
              ...
              <CodeBracketIcon size={20} className="mr-2" />
            )}
      ...
      <section className="grid md:grid-cols-2 gap-8 mb-16">
        <div className="p-6 bg-card-light dark:bg-card-dark rounded-lg shadow-lg">
          ...
          <p className="text-text-muted-light dark:text-text-muted-dark mb-4">
            Access all models and platform features through our robust APIs.
            Choose between REST for simplicity or GraphQL for flexible data
            fetching.
          </p>
          <a
            href="#"
            className="font-medium text-primary-DEFAULT hover:text-primary-dark dark:hover:text-primary-light"
          >
        ...
        <div className="p-6 bg-card-light dark:bg-card-dark rounded-lg shadow-lg">
          ...
          <p className="text-text-muted-light dark:text-text-muted-dark mb-4">
            Get started quickly with our official SDKs, available for popular
            languages.
          </p>
