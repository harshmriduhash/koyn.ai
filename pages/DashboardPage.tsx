      ...
      {apiKeys.length === 0 ? (
        ...
        <ul className="space-y-3">
          {apiKeys.map((key) => (
            <li
              key={key.id}
              className="p-3 border border-gray-200 dark:border-gray-700 rounded-md flex flex-col sm:flex-row justify-between items-start sm:items-center"
            >
              <div>
                <p className="font-mono text-sm text-text-light dark:text-text-dark break-all">
                  {key.key}
                </p>
              ...
              <div className="flex space-x-2 mt-2 sm:mt-0">
                <button
                  ...
                  className="p-1.5 text-text-muted-light dark:text-text-muted-dark hover:text-primary-DEFAULT dark:hover:text-primary-light rounded-md"
                  title="Copy API Key"
                >
                  {copiedKeyId === key.id ? (
                ...
                <button
                  ...
                  className="p-1.5 text-red-500 hover:text-red-700 dark:hover:text-red-400 rounded-md"
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-card-light dark:bg-card-dark p-6 rounded-lg shadow-xl w-full max-w-md">
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-text-muted-light dark:text-text-muted-dark mb-1">
                Scopes:
              </label>
              {/* Simplified scope selection; in a real app, this would be more detailed */}
              <div className="space-y-1">
                {["read:models", "predict:all", "manage:billing"].map(
                  (scope) => (
            ...
            <div className="flex justify-end space-x-3">
              <button
                ...
                className="px-4 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md text-text-light dark:text-text-dark hover:bg-gray-50 dark:hover:bg-gray-700"
              >
                Cancel
              </button>
              <button
                onClick={handleCreateKey}
                className="px-4 py-2 text-sm bg-primary-DEFAULT text-white rounded-md hover:bg-primary-dark"
              >
                Create Key
              </button>
            </div>
    ...
    <div className="p-6 bg-card-light dark:bg-card-dark rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-4">
        ...
        <select
          ...
          className="p-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-text-light dark:text-text-dark text-sm focus:ring-primary-DEFAULT focus:border-primary-DEFAULT"
        >
          ...
          <ResponsiveContainer>
            ...
            >
              ...
              <Line
                ...
                name="API Requests"
              />
              {usageData.some((d) => d.cost !== undefined) && (
                <Line
                  type="monotone"
                  dataKey="cost"
...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      document.title = `${
        currentUser.accountType === "creator" ? "Creator" : "Consumer"
      } Dashboard | ${APP_NAME}`;
    } else {
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="space-y-8">
          ...
          <>
            ...
            <section>
              <div className="flex items-center mb-4">
                <ChartBarIcon
                  size={24}
                  className="text-secondary-DEFAULT mr-3"
                />
                <h2 className="text-2xl font-semibold text-text-light dark:text-text-dark">
                  Usage Analytics
                </h2>
        ...
        {currentUser.accountType === "creator" && (
          <>
            <section>
              ...
              <p className="text-text-muted-light dark:text-text-muted-dark">
                Manage your published models and view their performance.
                (Creator model listing coming soon)
              </p>
            </section>
            <section>
              <div className="flex items-center mb-4">
              ...
              <p className="mt-2 text-sm text-text-muted-light dark:text-text-muted-dark">
                {" "}
                (Placeholder: Showing general usage. Creator-specific earnings
                and model performance charts coming soon.)
              </p>
            </section>
          </>
        )}
        ...
        <section className="p-6 bg-card-light dark:bg-card-dark rounded-lg shadow-md">
          ...
          <p className="text-text-muted-light dark:text-text-muted-dark">
            Update your profile, manage subscriptions, and billing information.
